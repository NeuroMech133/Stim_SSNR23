...
if checkEMG == 1
    for c = 1:size(ElChannelMap,2)
        ...
        for r = 1:size(ElChannelMap,1)
            plot(signal(ElChannelMap(r,c),:)/max(signal(ElChannelMap(r,c),:)) + r, 'Color', lincol(r,:), 'LineWidth', 1)
            grid on
            hold on
    ...
    if length(discardChannelsVec) == 65
        discardChannelsVec = discardChannelsVec(2:end);
    end
else
    if length(discardChannelsVec) == 65
